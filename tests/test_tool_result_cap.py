...
def test_per_result_cap_truncates_each_result(tmp_path):
    toolset = ToolSet([_BigTool()], work_dir=str(tmp_path), max_result_output_chars=100)
...
def test_per_result_cap_disabled_by_zero(tmp_path):
    toolset = ToolSet([_BigTool()], work_dir=str(tmp_path), max_result_output_chars=0)
    (only,) = _run(toolset, _msg(5000))
    assert only == "y" * 5000


def test_per_result_cap_leaves_small_results_alone(tmp_path):
    toolset = ToolSet([_BigTool()], work_dir=str(tmp_path), max_result_output_chars=1000)
...
def test_per_result_cap_stops_one_call_starving_the_rest(tmp_path):
    ...
    """
    starved = ToolSet([_BigTool()], work_dir=str(tmp_path), max_turn_output_chars=3000)
    first, second = _run(starved, _msg(5000, 5000))
    assert second.strip().startswith("<system-reminder>")  # nothing but the truncation notice

    fair = ToolSet(
        [_BigTool()], work_dir=str(tmp_path), max_turn_output_chars=3000, max_result_output_chars=1000
    )
...
def test_the_two_caps_say_which_one_truncated(tmp_path):
    ...
    the wrong fix.
    """
    per_result = ToolSet([_BigTool()], work_dir=str(tmp_path), max_result_output_chars=50)
    (out,) = _run(per_result, _msg(5000))
    assert "per-result limit" in out

    per_turn = ToolSet([_BigTool()], work_dir=str(tmp_path), max_turn_output_chars=50)
...
def test_turn_budget_still_applies_under_a_per_result_cap(tmp_path):
    ...
    toolset = ToolSet(
        [_BigTool()], work_dir=str(tmp_path), max_turn_output_chars=1000, max_result_output_chars=900
    )
