...
EXP_CKPT=${EXP_ROOT}/ckpt/fastcontext_sft_4b_slime/
EXP_HF_CKPT=${EXP_ROOT}/ckpt/fastcontext_sft_4b_hf/
mkdir -p ${EXP_CKPT}
mkdir -p ${EXP_HF_CKPT}

CKPT_ARGS=(
   --hf-checkpoint /mnt/local/models/Qwen3-4B-Instruct-2507/
   --ref-load /mnt/local/models/Qwen3-4B-Instruct-2507_torch_dist/
   --save ${EXP_CKPT}
   --save-interval 1000
   --rotary-base 5000000
)
...
if [ -n "${WANDB_KEY}" ]; then
   WANDB_ARGS=(
      ...
      --wandb-group qwen3-4B-instruct-sft-v3
      --wandb-key ${WANDB_KEY}
   )
else
   WANDB_ARGS=()
...
PYTHONPATH=/root/Megatron-LM python /root/slime/tools/convert_torch_dist_to_hf.py \
  ...
  --origin-hf-dir "/mnt/local/models/Qwen3-4B-Instruct-2507" \
  --force
